...
class Client extends EventEmitter {
      ...
      if (parsed.metadata.name.substr(0, 11) === 'data_packet') {
        ...
        encapsulatedPackets.forEach((encapsulatedPacket) => {
          if (encapsulatedPacket.hasSplit) {
            if (!this.splitPackets[encapsulatedPacket.splitID]) { this.splitPackets[encapsulatedPacket.splitID] = [] }
            ...
            if (encapsulatedPacket.splitCount === this.splitPackets[encapsulatedPacket.splitID].length) {
              let buffer = this.splitPackets[encapsulatedPacket.splitID].reduce((acc, bufferPart) => Buffer.concat([acc, bufferPart]), Buffer.alloc(0))
              delete this.splitPackets[encapsulatedPacket.splitID]
              this.readEncapsulatedPacket(buffer)
            }
          } else { this.readEncapsulatedPacket(encapsulatedPacket.buffer) }
        })
  ...
  readEncapsulatedPacket (buffer) {
    try {
      ...
      const r = this.encapsulatedPacketParser.parsePacketBuffer(buffer)
      this.emitPacket(r)
    } catch (err) {
      this.emit('error', err)
    }
  ...
  writeEncapsulatedRaw (buffer, options) {
    ...
    if ([2, 3, 4, 6, 7].indexOf(reliability) !== -1) {
      ...
      if (reliability === 3) { orderIndex = this.channelIndex[orderChannel]++ }
    }
